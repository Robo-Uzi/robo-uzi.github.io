...
I put the photo in `https://www.imagetotext.info/` and fixed the small errors it made in translating the text back to the correct characters.
```python
import re
...
```
{% endraw %}
After some time I have the flag. It didnt print the final `}` but I got the flag so didnt troubleshoot the script.
...
Visiting this link I see 6 check boxes. The site led me to http://challenge.nahamcon.com:31645/interesting where I decided to curl the `/interesting` endpoint with different http methods. After using `OPTIONS`, `HEAD`, `GET`, `POST`, `PUT`, `DELETE`, and `PATCH` I got the flag! I'm not sure which one didnt count.
