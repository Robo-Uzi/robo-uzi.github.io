...
author: robo.uzi
tags: [CTF]
permalink: /nullCTF-codename-neigh-2/
---
...
**Description**: Beyond the familiar chronometers lies the Temporal Nexus, where a desperate plea for the lost Neigh echoes through history. If the last trial felt light, prepare for forbidden archives, paradoxes, and enigmas that span epochs. [http://public.ctf.r0devnull.team:3003](http://public.ctf.r0devnull.team:3003) 

I get the new challenge files. The site looks exactly the same as in the first `Codename Neigh` challenge. The backend works slightly differently:
```shell
cat main.pony  
use "net"  
use "files"  
...
   end
```
The flag is now in `/private/flag.html`. 

I can make a request to `http://127.0.0.1/flag` the same way as in the first challenge:
```shell
...
```
I think this is unintended because its the same solve for both challenges. Now with 99% less unintended solves? Maybe lol.
