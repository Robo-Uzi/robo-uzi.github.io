...
author: robo.uzi
tags: [CTF]
permalink: /BITSCTF-2026-web-challenges/
---
* TOC
{:toc}
...
**Description**: I just vibecoded a highly secure reverse proxy using rust, I hope it works properly.[http://rusty-proxy.chals.bitskrieg.in:25001](http://rusty-proxy.chals.bitskrieg.in:25001)

I get the source code to the site. Contents of `main.rs`:
```rust
...
async fn handle_client(client: TcpStream, pool: Arc<ConnPool>) {
    ...
    loop {
        ...
        let mut backend_stream = match pool.acquire().await {
            Ok(s) => s,
            Err(e) => {
                let _ = send_error(&mut write_half, "502 Bad Gateway", "Backend unavailable.\n").await;
                break;
            }
        };
...
```
The proxy checks the path before URL decoding, and the backend routes after URL decoding. 

Url encoding the first character of `admin` allows the request to pass. Otherwise you get blocked:
```shell
curl "http://rusty-proxy.chals.bitskrieg.in:25001/admin/flag"  
Access denied.
```
